...
class AppointmentsJsonStore():
    ...
    class __AppointmentsJsonStore(JsonStore):
        ...
        _ID_FIELD2 = "_VaccinationAppointment__isCancel"
        ...
        def modificar_item(self, item, type_cancel):
            """Metodo encargado de modificar el contenido de un elemento de la lista de citas"""
            self.load()
            index = self._data_list.index(item)
            self._data_list[index][self._ID_FIELD2] = type_cancel
            self.save()

    __instance = None

    def __new__(cls):
        if not AppointmentsJsonStore.__instance:
            AppointmentsJsonStore.__instance = AppointmentsJsonStore.__AppointmentsJsonStore()
        return AppointmentsJsonStore.__instance
